...
test.init.gc <- function(obs, it){
  ...
  for(i in 1:it){
    print(i)
    set.seed(5*i+3)
    par <- c(runif(2), runif(3, 0, 6), runif(3,0,3))

    try(qN[i] <- quasi_newton(par, obs, modele.geyser.continu, lower = rep(0.01,8), upper = c(0.99,0.99, rep(Inf,6)))$counts[1])

    test.em <- function(par){
      em <- EM(par, obs, modele.geyser.continu, M.step.geyser.continu, max.iter = Inf, trace.theta = FALSE)
      return(em$iter)}
    try(BW[i] <- test.em(par))

    test.sq <- function(par){
      squarem <- SQUAREM(par, obs, modele.geyser.continu, M.step.geyser.continu, lower = rep(0,8), upper = c(1,1, rep(Inf,6)), max.iter = Inf, trace.theta = FALSE)
      return(squarem$iter)}
...
set.seed(28)
nb_it_gc <- test.init.gc(X.geyser, 5000)
saveRDS(nb_it_gc, "/home/sidonie/Bureau/github/steveHMM/examples/nb_it_geyser_continu.rds")
