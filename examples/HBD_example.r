...
X.HBD <- sample(1:3, 100, replace=TRUE) #we expect f ~= 0 since it is random (no consiguinity)
theta.HBD <- c(0.05,0.05)

em.HBD <- EM(theta.HBD, X.HBD, modele.HBD, M.step.HBD,max.iter = 100, trace.theta = TRUE)
qN.HBD <- quasi_newton(theta.HBD, X.HBD, modele.HBD, trace = TRUE, lower = c(0.01, 0.01), upper = c(0.99, Inf) )
